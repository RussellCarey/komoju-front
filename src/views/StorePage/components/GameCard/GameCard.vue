<template lang="">
	<div :class="'card-container'" :style="{ backgroundImage: 'url(' + image + ')' }" @mouseover="isHovering = true" @mouseleave="isHovering = false">
		...
		<div
			:data-cy="`add-to-favourite-${sluggedTitle}`"
			:class="isFavourite ? 'card-container-heart card-container-hearted' : 'card-container-heart'"
			@click="toggleFavourite"
		></div>
		<cartSVG :data-cy="`add-to-cart-${sluggedTitle}`" class="card-container-add" @click="addGameToCart" />
...
const sluggedTitle = props.title.split(" ").join("-")
